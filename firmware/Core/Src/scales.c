 ...
 * This MCU runs at 32MHz (31ns clock period), some extra delay is added by ASM NOPs.
 ...
 * There's the issue of the read routine being interrupted and the risk of the HX711s getting power down,
...
void HX711_readScales(HX711_TypeDef* lcell, HX711_TypeDef* rcell) {
	...
	// as long as the interrupt doesn't take longer than 60us, interrupts can be enabled
	// disable interrupts
	// noInterrupts();
...
void HX711_powerDown(HX711_TypeDef* cell) {
	...
	// it will power down if SCK remains for 60us high
}
